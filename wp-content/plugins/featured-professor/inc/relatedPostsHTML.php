...
function relatedPostsHTML($id){
  ...
  if ($postsAboutThisProf -> found_posts) {?>
    <p><?php echo wp_strip_all_tags(get_the_title()); ?> is mentioned in the following posts:</p>
    <ul>
      <?php
      while($postsAboutThisProf -> have_posts()) {
        $postsAboutThisProf->the_post(); ?>
        <li><a href="<?php the_permalink(); ?>"><?php echo wp_strip_all_tags(get_the_title()); ?></a></li>
